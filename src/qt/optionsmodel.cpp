...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeGelinAmount"))
        settings.setValue("nAnonymizeGelinAmount", 1000);
    nAnonymizeGelinAmount = settings.value("nAnonymizeGelinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeGelinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeGelinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeGelinAmount:
            return QVariant(nAnonymizeGelinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeGelinAmount:
            nAnonymizeGelinAmount = value.toInt();
            settings.setValue("nAnonymizeGelinAmount", nAnonymizeGelinAmount);
            emit AnonymizeGelinAmountChanged(nAnonymizeGelinAmount);
            break;
        default:
            break;
        }
