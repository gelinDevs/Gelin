...
void DarksendPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeGelinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeGelinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        //strAnonymizeGelinAmount = strAnonymizeGelinAmount.remove(strAnonymizeGelinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGelinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeGelinAmount*COIN) nMaxToAnonymize = nAnonymizeGelinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeGelinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeGelinAmount));
        //strAnonymizeGelinAmount = strAnonymizeGelinAmount.remove(strAnonymizeGelinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGelinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
